...
{ 
    ...
    {	
			...
			//Receive the three axis speed control command sent by the host through CAN communication
			//通过CAN通信接收主机发送过来的三轴速度控制命令
...
//					else if (Car_Mode==5)	 Set_Pwm(-MOTOR_A.Motor_Pwm,-MOTOR_B.Motor_Pwm,-MOTOR_C.Motor_Pwm,-MOTOR_D.Motor_Pwm);    //顶配麦轮独立悬挂常规型 TOP_MEC_DL    MD60N_18
          ...
          //Eight_drive_SENIOR_MEC_DL - 8驱_高配麦轮独立悬挂常规型 MD36N_27 
				 	if      (Car_Mode<=6)	 Set_Pwm( MOTOR_A.Motor_Pwm, MOTOR_B.Motor_Pwm, MOTOR_C.Motor_Pwm, MOTOR_D.Motor_Pwm);  
				 	//Eight_drive_TOP_MEC_DL    - 8驱_顶配麦轮独立悬挂常规型 MD60N_18 
          ...
          else if (Car_Mode==1||2)                        Set_Pwm(-MOTOR_A.Motor_Pwm,-MOTOR_B.Motor_Pwm,-MOTOR_C.Motor_Pwm,0); //高配全向轮圆形/三角形常规 SENIOR_OMNI MD36N_27
...
void Remote_Control(void)
{
    ...
    Move_X= Move_X*RC_Velocity/500; 
		Move_Y= Move_Y*RC_Velocity/500;
		Move_Z= Move_Z*(PI/4)*(RC_Velocity/500)/500;  
...
{
	static u8 error=0;
	if(abs(MOTOR_A.Motor_Pwm)>1000||abs(MOTOR_B.Motor_Pwm)>1000||abs(MOTOR_C.Motor_Pwm)>1000||abs(MOTOR_D.Motor_Pwm)>1000)   error++;
	else error=0;
