...
void CAN_SEND(void) 
{
	...
	CAN_X_speed=Move_X*1000;
	CAN_Y_speed=Move_Y*1000;
	CAN_Z_speed=Move_Z*1000;
	
	//The host sends the XYZ triaxial target speed control command to the sending slave
	//主机向发送从机发送XYZ三轴目标速度控制命令
	CAN_SENT[0]=CAN_X_speed>>8;
	CAN_SENT[1]=CAN_X_speed;
	CAN_SENT[2]=CAN_Y_speed>>8;
	CAN_SENT[3]=CAN_Y_speed;
	CAN_SENT[4]=CAN_Z_speed>>8;
	CAN_SENT[5]=CAN_Z_speed;	
	CAN_SENT[6]=0;               
	CAN_SENT[7]=0;              
	if(Turn_Off(Voltage)==1 || robot_mode_check_flag==1)
	{
		CAN_SENT[0]=0;
		CAN_SENT[1]=0;
		CAN_SENT[2]=0;
