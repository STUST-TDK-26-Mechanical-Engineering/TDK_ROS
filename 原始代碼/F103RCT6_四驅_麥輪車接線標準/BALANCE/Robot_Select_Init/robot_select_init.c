...
void Robot_Select(void)
{
  ...
  //ADC值分段变量，取决于小车型号数量
	...
	//四驱一共有12种车型，其中电位器可调节到12，为定制车预留车型
	Car_Mode=(int) (Get_adc_Average(CAR_MODE_ADC,10)/Divisor_Mode); //Collect the pin information of potentiometer //采集电位器引脚信息		

	#if _4WD
	{	
		if (Car_Mode==0)   Robot_Init(SENIOR_4WD_BS_wheelspacing, SENIOR_4WD_BS_axlespacing, MD36N_27, Photoelectric_500, _4WD_225); //SENIOR_4WD_BS      - 高配4驱摆式悬挂常规型 //BS: Pendulum suspension
		if (Car_Mode==1)   Robot_Init(SENIOR_4WD_BS_wheelspacing, SENIOR_4WD_BS_axlespacing, MD36N_51, Photoelectric_500, _4WD_225); //SENIOR_4WD_BS      - 高配4驱摆式悬挂重载型  
		
		if (Car_Mode==2)   Robot_Init(SENIOR_4WD_DL_wheelspacing, SENIOR_4WD_DL_axlespacing, MD36N_27, Photoelectric_500, _4WD_225); //SENIOR_4WD_DL      - 高配4驱独立悬挂常规型 //DL: Independent suspension 
		if (Car_Mode==3)   Robot_Init(SENIOR_4WD_DL_wheelspacing, SENIOR_4WD_DL_axlespacing, MD36N_51, Photoelectric_500, _4WD_225); //SENIOR_4WD_DL      - 高配4驱独立悬挂重载型
		...
		if (Car_Mode==8)   Robot_Init(FlagShip_4WD_BS_wheelspacing, FlagShip_4WD_BS_axlespacing, MD60N_18, Photoelectric_500, _4WD_250); //FlagShip_4WD_BS    - 旗舰4驱摆式悬挂常规型
		if (Car_Mode==9)   Robot_Init(FlagShip_4WD_BS_wheelspacing, FlagShip_4WD_BS_axlespacing, MD60N_47, Photoelectric_500, _4WD_250); //FlagShip_4WD_BS    - 旗舰4驱摆式悬挂重载型
